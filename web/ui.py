...
def header_html(days=0, active_name="", is_admin=False, show_insights=True):
    ...
    return (
        ...
        # Insights progress modal — pure vanilla JS, no Alpine dependency.
        # All "Generate Insights" buttons dispatch the open-insights-modal
        # CustomEvent on window; this listener catches it, opens the modal,
        # connects the SSE stream, and htmx-reloads the panel when done.
        '<div id="insights-modal" style="display:none" '
        'class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">'
        '<div class="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">'
        '<div class="mb-4">'
        '<div class="inline-block w-10 h-10 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mb-3"></div>'
        '<h3 class="text-white font-semibold text-lg mb-1">Generating Insights</h3>'
        '<p id="im-status-text" class="text-gray-400 text-sm">Connecting to model...</p>'
        '</div>'
        '<div class="bg-gray-800 rounded-lg p-3 text-left">'
        '<p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Status</p>'
        '<p id="im-status-code" class="text-emerald-400 text-sm font-mono">initializing</p>'
        '</div>'
        '<p class="text-gray-600 text-xs mt-4">Using local model &middot; this may take a moment</p>'
        '</div></div>'
        '<script>'
        '(function(){'
        'var _es=null;'
        'function _show(){document.getElementById("insights-modal").style.display="flex";}'
        'function _hide(){document.getElementById("insights-modal").style.display="none";}'
        'function _setText(id,v){var el=document.getElementById(id);if(el)el.textContent=v;}'
        'function startInsightsStream(detail){'
        '_show();'
        '_setText("im-status-text","Connecting to model...");'
        '_setText("im-status-code","starting");'
        'if(_es){_es.close();_es=null;}'
        '_es=new EventSource(detail.streamUrl);'
        '_es.onmessage=function(e){'
        'try{var msg=JSON.parse(e.data);}catch(_){return;}'
        'if(msg.status){_setText("im-status-code",msg.status);_setText("im-status-text","Running analysis...");}'
        'if(msg.done){'
        '_es.close();_es=null;'
        '_hide();'
        'if(detail.target&&detail.reloadUrl){'
        'htmx.ajax("GET",detail.reloadUrl,{target:"#"+detail.target,swap:"innerHTML"});'
        '}'
        '}'
        '};'
        ...
        'if(_es){_es.close();_es=null;}'
        '_hide();'
        'toast("Insights generation failed — check model connection","err");'
        '};'
        '}'
        'window.startInsightsStream=startInsightsStream;'
        'window.addEventListener("open-insights-modal",function(e){startInsightsStream(e.detail);});'
        '})()'
        '</script>'
