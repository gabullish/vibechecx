...
def header_html(days=0, active_name="", is_admin=False, show_insights=True):
    ...
    return (
        ...
        # Insights progress modal — shown when user triggers insight generation
        # via the SSE streaming endpoint. Alpine.js component listens for the
        # open-insights-modal custom event dispatched by the generate buttons.
        '<div id="insights-modal" '
        'x-data="insightsModal()" '
        'x-show="open" '
        'x-cloak '
        'class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" '
        '@open-insights-modal.window="startStream($event.detail)" '
        '@close-insights-modal.window="open = false">'
        ...
        '<p class="text-gray-400 text-sm" x-text="statusText"></p>'
        '</div>'
        '<div class="bg-gray-800 rounded-lg p-3 text-left">'
        '<p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Status</p>'
        '<p class="text-emerald-400 text-sm font-mono" x-text="currentStatus"></p>'
        '</div>'
        ...
        'return{'
        'open:false,'
        'statusText:"Connecting to local model...",'
        'currentStatus:"waiting",'
        '_es:null,'
        '_target:null,'
        '_reloadUrl:null,'
        'startStream:function(detail){'
        'this.open=true;'
        'this.statusText="Connecting to local model...";'
        'this.currentStatus="initializing";'
        'this._target=detail.target;'
        'this._reloadUrl=detail.reloadUrl;'
        'var self=this;'
        'if(this._es)this._es.close();'
        'this._es=new EventSource(detail.streamUrl);'
        'this._es.onmessage=function(e){'
        'var msg=JSON.parse(e.data);'
        'if(msg.done){'
        'self._es.close();'
        'self.open=false;'
        'if(self._target&&self._reloadUrl){'
        'htmx.ajax("GET",self._reloadUrl,{target:"#"+self._target,swap:"innerHTML"});'
        '}'
        '}else{'
        'if(msg.status){self.currentStatus=msg.status;self.statusText="Running analysis...";}'
        '}'
        '};'
        'this._es.onerror=function(){'
        'self._es.close();self.open=false;'
        '};'
        '}'
        '};'
        '}'
